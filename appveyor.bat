...
where vcpkg >nul 2>&1
if not %errorlevel% neq 0 goto :skipvcpkg
...
if /I "%PLATFORM%"=="arm64" (
  ...
  call :mklink vim\runtime\GvimExt64\libiconv-2.dll ^
    vim\runtime\libiconv-2.dll
  call :mklink vim\runtime\GvimExt64\libintl-8.dll ^
  ...
  if "%INCLUDE_LIBGCC%-%BIT%"=="1-32" (
    call :mklink vim\runtime\libgcc_s_sjlj-1.dll ^
      %DEPENDENCIES%\gettext32\libgcc_s_sjlj-1.dll
  )

  call :mklink vim\runtime\GvimExt32\libiconv-2.dll ^
    %DEPENDENCIES%\gettext32\libiconv-2.dll
  call :mklink vim\runtime\GvimExt32\libintl-8.dll ^
    %DEPENDENCIES%\gettext32\libintl-8.dll
  if "%INCLUDE_LIBGCC%"=="1" (
    call :mklink vim\runtime\GvimExt32\libgcc_s_sjlj-1.dll ^
      %DEPENDENCIES%\gettext32\libgcc_s_sjlj-1.dll
  )
)
...
vcpkg install %1:arm64-windows --vcpkg-root %DEPENDENCIES%\vcpkg && exit /B
@goto :eof
